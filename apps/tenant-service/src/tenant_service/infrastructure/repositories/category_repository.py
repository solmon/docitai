...
class DocumentCategoryRepository:
    ...
    async def get_by_id(self, category_id: UUID, tenant_id: UUID) -> Optional[DocumentCategory]:
    ...
    async def list_by_tenant(self, tenant_id: UUID, skip: int = 0, limit: int = 50) -> list[DocumentCategory]:
    ...
    async def list_by_parent(self, parent_id: UUID, tenant_id: UUID) -> list[DocumentCategory]:
    ...
    async def get_category_tree(self, tenant_id: UUID, parent_id: Optional[UUID] = None) -> list[DocumentCategory]:
    ...
    async def get_descendants(self, parent_id: UUID, tenant_id: UUID) -> list[DocumentCategory]:
        ...
        while to_process:
            ...
            children = [c for c in all_categories if c.parent_category_id == current_id and c.deleted_at is None]
